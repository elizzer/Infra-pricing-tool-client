...
const Home: React.FC<Props> = () => {
    ...
    if(code){
        ...
        .then((res)=>{
            ...
            if(!res.error){
                localStorage.setItem("userData",`${res.user.email}:${res.user.name}:${res.user._id}`)
    ...
    function loginHandler(e: React.FormEvent<HTMLFormElement>): void {
        ...
        .then(res => {
            ...
            if(res.error){
                ...
                setLoginError('');
                //login success
                localStorage.setItem("userData",`${res.userData.email}:${res.userData.name}:${res.userData._id}`)
                router.push("/dashboard")
            }
        })
        .catch(error => {
    ...
    return (
        <div className="h-screen flex flex-col justify-center items-center bg-gray-100">
            <div className="max-w-md w-full bg-white rounded-lg shadow-md p-8">
                <h1 className="text-3xl font-bold text-center mb-6 text-black">Welcome to our Login Page</h1>
