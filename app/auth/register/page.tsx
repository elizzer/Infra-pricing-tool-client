...
const Register: React.FC<Props> = () => {
    ...
    function registerHandler(e: React.FormEvent<HTMLFormElement>): void {
        ...
        fetch(`${process.env.NEXT_PUBLIC_SERVER_ENDPOINT}/user/auth/register`,{
            ...
            if(!res.error){
                localStorage.setItem("userData",`${res.user.email}:${res.user.name}`)
        ...
        <div className="h-screen flex flex-col justify-center items-center bg-gray-100">
            <div className="max-w-md w-full bg-white rounded-lg shadow-md p-8">
                ...
                <form onSubmit={registerHandler}>
                    <div className="mb-6">
                        <label htmlFor="email" className="block text-lg font-semibold mb-2 text-black">Email:</label>
                        <input
                            type="email"
                            id="email"
                            onChange={emailHandler}
                            value={email}
                            className={`w-full text-black bg-gray-100 border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 ${emailError ? 'border-red-500' : ''}`}
                        />
                        {emailError && <p className="text-red-500 mt-1">{emailError}</p>}
                    </div>
                    <div className="mb-6">
                        <label htmlFor="name" className="block text-lg font-semibold mb-2 text-black">Name:</label>
                        <input
                            type="text"
                            id="name"
                            onChange={nameHandler}
                            value={name}
                            className={`w-full text-black bg-gray-100 border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 ${nameError ? 'border-red-500' : ''}`}
                        />
                        {nameError && <p className="text-red-500 mt-1">{nameError}</p>}
                    </div>
                    <div className="mb-6">
                        <label htmlFor="password" className="block text-lg font-semibold mb-2 text-black">Password:</label>
                        <input
                            type="password"
                            id="password"
                            onChange={passwordHandler}
                            value={password}
                            className={`w-full text-black bg-gray-100 border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 ${passwordError ? 'border-red-500' : ''}`}
                        />
